...
// Intercept process.exit in NWJS context to prevent killing the GUI process
if (typeof process !== 'undefined' && process.versions && process.versions.nw) {
	var _originalExit = process.exit;
	process.exit = function(code) {
		console.log("[NWJS] process.exit(" + code + ") intercepted - server will not exit the GUI process");
	};
}
...
if (!fs.existsSync(configPath)) {
	console.error("config.yaml not found. Please create one based on the default config.");
	process.exit(1);
}
var config = yaml.load(fs.readFileSync(configPath, "utf8"));
var serverCfg = config.server || {};
var dataCfg = config.data || {};
var ServerPort = serverCfg.port || 4288;
var BindAddress = serverCfg.bind_address || "127.0.0.1";
var DATA_DIR = path.resolve(dataCfg.directory || path.join(__dirname, "data"));
fs.mkdirSync(DATA_DIR, { recursive: true });

// Initialize SQLite database (replaces MongoDB)
var SqliteDB = require("./database");
/** @type {SqliteDB} */
var Database;
var AccountCollection = "Accounts";
var NextMemberNumber = 1;

// Will be set after async init
...
var MaxHeapUsage = 16_000_000_000;
/** @type {Account[]} */
var Account = [];
/** @type {Chatroom[]} */
var ChatRoom = [];
...
// Server port from config (already loaded above)
...
try {
	...
	var mailPass = process.env.EMAIL_PASSWORD || "";
	if (mailPass) {
		...
		console.log("Email transport configured for password reset");
	}
} catch (e) {
	console.log("Email transport not available (nodemailer not installed or not configured)");
}

// If the server received an unhandled error, we log it and exit
...
process.on('uncaughtException', function(error) {
	...
	console.log("*************************");
	try { AccountDelayedUpdate(); } catch (e) { console.log("Error while doing delayed updates"); }
	process.exit(1);
});
...
(async function initServer() {
	Database = await SqliteDB.init(DATA_DIR);

	var result = Database.db.exec("SELECT MAX(MemberNumber) as m FROM accounts");
	if (result.length > 0 && result[0].values.length > 0 && result[0].values[0][0] != null) {
		NextMemberNumber = result[0].values[0][0] + 1;
	}
	console.log("Next Member Number: " + NextMemberNumber);
	console.log("****************************************");
	console.log("Database: SQLite connected (" + DATA_DIR + "/bondage_club.db)");

	App.listen(ServerPort, BindAddress, function () {
		console.log("Bondage Club Lite server is listening on " + BindAddress + ":" + (ServerPort).toString());
		console.log("****************************************");
		...
		socket.once("disconnect", () => {
			...
			if (sameIPConnectionsDisconnect.length <= 1) {
				...
				sameIPConnectionsDisconnect.shift();
	...
	_intervals.push(setInterval(AccountSendServerInfo, 60000));
	_intervals.push(setInterval(AccountDelayedUpdate, 300000));

	});
})().catch(function(err) { console.error("Failed to initialize server:", err); process.exit(1); });
...
function AccountSendServerInfo(socket) {
	...
	if ((MemoryData != null) && (MemoryData.heapUsed > MaxHeapUsage)) {
		console.log("Heap usage exceeded limit, restarting...");
		try { AccountDelayedUpdate(); } catch (e) { console.log("Error while doing delayed updates"); }
		process.exit(2);
	}
...
function PasswordReset(data, socket) {
	if ((data != null) && (typeof data === "string") && (data != "") && CommonEmailIsValid(data)) {
		...
		if (!MailTransporter) {
			return socket.emit("PasswordResetResponse", "EmailSentError");
		}
...
// Module exports for NWJS GUI integration
if (typeof module !== 'undefined' && module.exports) {
	module.exports = {
		stop: function(callback) {
			...
			_intervals = [];
			try { AccountDelayedUpdate(); } catch (e) { console.log("Error during delayed updates: " + e.message); }
			for (var j = 0; j < Account.length; j++) {
				if (Account[j] != null && Account[j].Socket != null) {
					try { Account[j].Socket.emit("ServerMessage", "Server is shutting down."); } catch (e) {}
				}
			}
